...
| **hydrated** | server catch-up applied locally | `SYNC_CAUGHT_UP` marker frame (`packages/contract/src/index.ts:149`, sent by the DO at `apps/worker/src/doc-do.ts:522` after the one-frame snapshot catch-up (PR #134); client flips to `"live"` only on receiving it, `apps/web/src/store/doc-connection.ts:296`) |
| **durable** | content persisted server-side | server-side `seedDoc` RPC before any client connects (doc-do.ts:294, no-clobber) |
| **broadcast** | other connected clients told | explicit `broadcast` after persist (seedDoc broadcasts its seed, doc-do.ts:308) |
...
1. For any async resource, enumerate every state a peer could observe — including the
   ...
   `apps/web/src/store/routine.ts:72`).
...
- c43ebed: connect catch-up called `getDoc()`, which auto-materialized **and
  ...
  auto-materialize) on the catch-up path (doc-do.ts:195) + seedDoc broadcasts.
...
change log ≠ live doc; fixed by PR #139/903d109 — `migrateOnLoad` adopts the migrated doc)
— see ballroom-flow-failure-archaeology pattern 2 and ballroom-flow-v5-migration-campaign
§2 before touching the DO load path.
...
Authored 2026-07-02 against repo HEAD 70eed7e; line anchors refreshed 2026-07-02 — **verified
at HEAD `c9622c9`** (post-#139/#136/#137; the migrateOnLoad fix merged as PR #139, #140
closed as superseded) on `development`. Verified directly:
...
`apps/worker/src/doc-do.ts` `commenterChangeAllowed` :579 / `refreshConnectedRoles`
:638 / SYNC_CAUGHT_UP send :522; `test-support/explain.ts` in full;
