...
description: Load when working on or building atop the v5 live-figure migration (formerly PLAN §9, now closed history — see docs/README.md § For historians, and docs/concepts/figures.md § Variants for the current model) — COMPLETE as of 2026-07-02 (HEAD 759b3a8) — global figure docs, overlay/variant resolution in the web store, library-as-bookmark, admin seams, figure-scoped undo, or verifying the shipped steps still hold. Also load when a task mentions "variant", "spawnVariant", "resolveFigure", "frozen copy", "per-beat ownership", or the "Passing Tumble Turn".
---

# The v5 live-figure migration campaign

**Status (2026-07-02, HEAD `759b3a8`): the milestone is COMPLETE — zero `☐` boxes left in
the old PLAN §9** (that document was dissolved 2026-07-15 into `docs/README.md` +
`docs/concepts/` + `docs/system/`; the roadmap is now closed history — see `docs/README.md`
§ For historians). The final box (figure-editor undo targets the figure doc, formerly §5.4,
now `docs/concepts/collaboration.md` § Undo) shipped in PR #141
...
**When NOT to use this:** for CRDT/Automerge concepts (per-beat ownership mechanics, undo internals, convergence testing) read **ballroom-flow-crdt-reference**; for the frozen module boundaries and locked decisions read **ballroom-flow-architecture-contract**; for how to run/test anything read **ballroom-flow-build-and-env** and **ballroom-flow-validation-and-qa**; for PR/TDD/doc-update doctrine read **ballroom-flow-change-control**; for debugging a failure you didn't cause read **ballroom-flow-debugging-playbook**; for seeding/charting figure content read **ballroom-flow-figure-data-pipeline**.

## 1. Mission context (read once)

- **v5 = live figures + per-beat overlay variants** (formerly PLAN §5.2, §2.5.1 invariants #14–18, now `docs/concepts/figures.md` § Variants). A placed catalog figure is a **live reference** to a global, admin-owned figure doc. A non-admin's first edit **spawns a variant**: an account figure whose `baseFigureRef` is a *live* link, carrying **only the beats it owns**; everything else resolves live from the base.
- **Canonical scenario (formerly PLAN §5.2, now `docs/concepts/figures.md` § Variants):** a Slowfox choreo places the catalog *Tumble Turn* twice — one plain, one re-choreographed as a *Passing Tumble Turn* (last ~3 beats changed). When the catalog figure later gains new attribute values, the plain placement shows them everywhere; the Passing variant shows them **only on its untouched beats**.
...
- **Back-compat guarantee (formerly PLAN §9, line ~456, now folded into `docs/concepts/figures.md` § Variants): NO data migration.** A v4 frozen copy carries content on every beat it uses, so it **owns every such beat** (invariant #15) and `resolveFigure(base, copy)` returns exactly its current timeline. Its `baseFigureRef` becoming live changes nothing until the catalog adds values on beats the copy never touched. Existing catalog-seeded placements keep their account docs; **only new catalog adds become live references**.

## 2. Current-state audit (verified at HEAD 759b3a8, 2026-07-02)

The old PLAN §9 (lines ~445–456) used to carry the authoritative per-step detail; that
roadmap is now closed history (see `docs/README.md` § For historians) and this table is the
...
- **Branch off `main`** (the sole trunk since `development` was deleted 2026-07-05, PR #161 — historically the rule was "never `main`, always `development`", after the #83/#85 revert cost ~1269 lines building on a stale `main`; the roles have since swapped). PR back into `main`.
- **The docs move with the code:** reconcile any superseded prose in the owning `docs/concepts/`/`docs/system/` doc (and docs/TEST-MAP.md rows) **in the same PR** (formerly "flip the matching PLAN §9 checkbox" — that roadmap is closed history). Divergence between the docs and code is a bug.
...
```bash
git branch --show-current                       # must be a feature branch off main (development was deleted 2026-07-05)
grep -n '☐' docs/PLAN.md 2>/dev/null || echo "docs/PLAN.md is gone (dissolved 2026-07-15) — ZERO boxes remained at 759b3a8; any new milestone work is tracked elsewhere now, see docs/README.md"
pnpm --filter @weavesteps/domain test             # baseline green before you touch anything
```

Read `docs/concepts/figures.md` § Variants (formerly PLAN §5.2), `docs/concepts/collaboration.md`
§ Undo (formerly §5.4), and the invariants (formerly §2.5.1 #14–20, now `docs/concepts/figures.md`
§ Variants) in full.

### How the last box shipped (PR #141 — for anyone touching undo next)

`docs/concepts/collaboration.md` § Undo (LOCKED, formerly PLAN §5.4): "**Undo follows the surface being edited**: in the Assemble view it
...
| Do NOT | Why |
|---|---|
| Create frozen copies for new divergence | Retired by PLAN v5.0 (D12 ⟳v5, now `docs/concepts/figures.md` § Variants). `copyOnWrite` is legacy-read-only; new divergence goes through `spawnVariant` |
| Mutate the base on a variant edit, or let a base edit rewrite owned beats | Invariant #17 (formerly PLAN §2.5.1, now `docs/concepts/figures.md` § Variants). `spawnVariant`/`variantAttributesForEdit` are pure — keep it that way through the store |
| Write a data migration for v4 frozen copies | The back-compat guarantee makes it unnecessary (formerly PLAN §9, now `docs/concepts/figures.md` § Variants); a migration adds risk for zero benefit |
...
| Bypass the DO boundary for admin writes (e.g. direct D1/SQLite edits of doc content) | Permissions are enforced per-document at the DO sync boundary (formerly PLAN §5.1, now `docs/concepts/collaboration.md` § Roles, enforcement in `docs/system/architecture.md` § Permission enforcement; CLAUDE.md §4); D1 is a derived index, never the source of truth |
...
| A v4 routine renders differently after your change | Back-compat broken — a full-timeline copy must resolve to exactly itself | Add/restore the v4-shape pin test before anything else (formerly PLAN §9 back-compat note, now `docs/concepts/figures.md` § Variants) |
...
- **Invariant tests:** invariants #14–18 (formerly PLAN §2.5.1, now `docs/concepts/figures.md` § Variants) stay pinned in `packages/domain/src/fork.test.ts` + `convergence.test.ts`; any new store/worker resolution path gets its own back-compat pin (v4-shape figure resolves to itself).
...
- **PR hygiene:** branch off `main` (`development` was deleted 2026-07-05); the owning `docs/concepts/`/`docs/system/` doc (formerly the PLAN §9 checkbox) + TEST-MAP row flipped in the same PR; feature is "done" only when its journey is green on PR.
...
c9622c9) on `development` (the trunk at the time — merged into `main` and deleted from the
remote 2026-07-05, PR #161; `main` is the sole trunk now). All file:line claims, route paths, suite counts (domain 245/3
...
`docs/PLAN.md` itself was dissolved 2026-07-15 into `docs/README.md` + `docs/concepts/` +
`docs/system/`; every PLAN §/D-number citation in this file has been repointed to its new
home (see `docs/README.md` § For historians for the full citation map) while the historical
facts (dates, commits, PR numbers) are preserved as-is.

Re-verify before relying on the audit:
```bash
grep -rn "zero" docs/README.md                                     # the v5/PLAN §9 roadmap is closed history — no live boxes to grep
