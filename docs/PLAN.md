...
Standard kinds (v1): **`direction`** (the step headline — **closed enum** `forward`/`back`/`side`/`behind`/`close`/`diagonal`/`in_place`; the legacy split diagonal `diag_forward`/`diag_back`→`diagonal` normalizes on read) `#2f5d8f`; **`footwork`** (the foot part — **free-text**, suggested set the compound ISTD codes `HT`/`T`/`TH`/`H`/`heel pull`; any legacy anatomical value (`ball`/`heel`/`toe`/…) still validates + displays as free-text — `H`/`T` are now canonical picker values, NOT rewritten on read) `#a9742c`; **`rise`** (`commence`/`body_rise`/`foot_rise`/`up`/`continue`/`lowering`/`NFR`; **Tango omits** via `appliesToDances`) `#1f8a5b`; **`position`** (single: `closed`/`promenade`/`wing`/`CBMP` — CBMP is a *position*, not a body action) + **`bodyActions`** (multi: `CBM`) `#8a5cab`; **`sway`** (`to_L`/`to_R`/`none`) `#c0563f`; **`turn`** (`eighth_L`…`half_R`/`none`) `#5b6b8a`.
