...
- **Reading-view column filter — "type chips" (design 1.23/1.26, ✓ decided 2026-07-03, direction A of the 1d "too crowded" exploration; B's view-sheet stays a future option if types multiply, C dropped).** A chips row at the top of the reading programme — one chip per attribute type **used anywhere in the routine** (custom kinds first-class, shown with their stored colour + dot) — hides/shows that technique **column across every figure**. Rules (LOCKED): **Step\* is required, never hideable** (tapping it toasts *"Step is always shown"*); off = **dashed grey** (grey stays "empty/off", never data); **default: everything on** — the row only ever reduces; remembered **per device, across choreos** (`bb_hidden_types` — exactly like `bb_role`); **hiding never touches data** — notes, breaks and whole-figure comments are never filtered; tap a value/column header → explainer (frame 1.13) unchanged. A figure using hidden types carries a **"+N hidden"** pill on its type row: tap to **peek** (that one figure expands, chips stay put, label flips to "– hide"), collapsing on the next scroll. The **Leader/Follower toggle moved into the screen header as a compact muted L·F** (full accessible names kept), so the chips row replaces the old STEPS FOR row — no net extra chrome. Discoverability: a dedicated tour stop (4 of 5, after the header stops), plus a **one-time toast** for tour skippers the first time a view hides data (*"Some columns are tucked away — '+N hidden' shows them"*, `bb_hidden_types_hint`, also stamped when the user hides a column themselves).
...
- **US-054 (book-verified full-syllabus seed)** — blocked on the external ISTD/WDSF technique books (content workstream below); not engineering work.
...
- **Charted so far (research-derived, source-attributed):** the charting effort has gone far past the original hand-charted core — **`figure-charts.generated.ts` now carries 294 per-step both-role charts** (waltz 66, foxtrot 76, quickstep 76, tango 64, viennese waltz 12), generated by `scripts/gen-figure-charts.mjs` from `docs/seed/figure-charts.json` (WDSF-first, one recorded source per figure; no fabrication — unverifiable figures were removed, not guessed). **Book-verification is an external dependency**: the paid ISTD/WDSF technique books are on order (owner), and until they arrive the charts remain research-derived and want a dancer's check — this is exactly the "refine with real testers" posture below, **not a launch blocker and not actionable engineering work now** (US-054, the formal book-verified full-syllabus seed, stays skipped until the books are in hand). **`docs/seed/figure-charts.json` is the single place to extend** — a guard test keeps each entry's step count aligned to its WDSF timing so it can't silently fall back.
