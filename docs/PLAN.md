...
| **0.5** | **Architecture spike — ✅ DONE (GO)** | **Ran it** (`apps/worker/spike/`, real workerd+DO+SQLite via vitest-pool-workers): Automerge runs in workerd, persists to DO SQLite + reloads, two clients converge, permission boundary holds, multi-doc + variant-overlay (base flows up) work, and it bundles to **920 KiB gzip** (well under the paid limit). **Verdict: GO.** Findings + sharp edges → [`docs/spike/SPIKE-FINDINGS.md`](spike/SPIKE-FINDINGS.md). One unknown deferred to M2: live WebSocket/hibernation sync (not testable in vitest). |
...
**M0.5 exit — ✅ met (GO).** The throwaway (`apps/worker/spike/`) proved S1–S5 + smoke on the real runtime and a deployable 920 KiB-gzip bundle. **Findings that refined the plan** (full write-up in [`docs/spike/SPIKE-FINDINGS.md`](../spike/SPIKE-FINDINGS.md)):
