...
### 4.2 Figure Library — browse the **application-global library** (canonical figures, grouped by `figureType`, filterable by dance) and **your personal-library figures** (custom badge shows source lineage); create/edit; "used in N routines". Editing one of *your personal-library* figures flows into all your referencing routines; editing a **global** one, or a personal-library figure inside a choreo, is **copy-on-write to a frozen choreo-owned copy**. From a figure family you can open the **cross-dance note** surface (annotate this dance or all dances — §4.6).
...
- **Personal library (explicit reuse):** from a choreo figure a user may **"save to my library"** → a personal-library figure (account-scoped, owned by them) reusable across **their** routines; editing it in place then flows to all of **their** routines that reference it (US-034). Placing a personal-library figure into a *different* choreo and editing it triggers copy-on-write (it lives outside that choreo).
