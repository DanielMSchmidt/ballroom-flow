...
> - **Architecture (v3):** the data model is now a **CRDT document per routine in a Durable Object** (online-first), **not** D1-rows + server-LWW. This **reverses two things this document asserts**: (1) it says "**NO offline/CRDT two-client merge tests**" — under v3 the **CRDT convergence/merge + sync-boundary permission tests are back** (PLAN.md §10.2); (2) it tests a bespoke **op-log/footprint undo + LWW** — that machinery is **dropped** in favour of the CRDT's native per-user undo, so those rows no longer apply.
>
