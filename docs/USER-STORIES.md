...
**Derived from:** `docs/PLAN.md` (v5.0)
**Status:** Backlog for v1. Incrementally ordered, mapped to the M1–M9 roadmap (§9).

> **⟳v5 reconciliation (2026-07-02).** PLAN v5.0 reverses the 2026-06 frozen-copy model: figures are **live wherever referenced**, a **variant** is a live overlay over its base (per-beat ownership + copy-down), global figures are **real admin-owned docs**, the library is a **per-user bookmark** over shared docs, and a fork **copies account figures** while keeping catalog refs live (PLAN §5.2, D12/D28/D30/D31). Stories written against the frozen-copy semantics — **US-008, US-033, US-034, US-035** — describe shipped v4.x behavior and are **superseded in place by US-057…US-061 below** (IDs never reused; the old text is kept for lineage). The engineering sequence is PLAN §9 "v5 migration milestone".
...
| **FE-3 Figures, copies & fork** | `fork-and-figures.spec` — fork choreo (frozen); edit shared figure → COW frozen copy; figure auto-updates across routines | ~~US-006~~ (retired) · US-007/008 ✓ (domain) · **US-037 choreo fork ✓** · **US-028/029/030 notate-a-figure ✓ (wired into Assemble + authoring journey)** · **US-031 alignment ✓** · **US-032 figure library browse + add-from-library picker ✓** · **US-033/034/035/036 personal-library + COW frozen-copy + figure auto-update ✓ (PR #74)** · #42, #56 | **built** — notation + library + alignment + choreo fork + figure auto-update + copy-on-write (frozen choreo-owned copy, "copied into this choreo") all landed; `fork-and-figures.spec` (US-034/035/037 `@smoke`) active in the E2E gate. *(Reconciled 2026-06: "auto-variant" → "frozen copy"; US-006 overlay model retired.)* **⟳v5 (2026-07-02): the frozen-copy semantics this epic shipped are superseded — see FE-9 below.** |
| **FE-9 Live figures & variants (⟳v5)** | extends `fork-and-figures.spec` — catalog placement is a live ref; edit catalog figure → live variant (base edit appears on untouched beats only); shared figure edits flow into every referencing choreo; add-to-my-library bookmark (partner bookmarks the same doc); fork copies account figures | **US-057 variant/overlay (domain)** · **US-058 global figure docs + admin boundary** · **US-059 library-as-bookmark** · **US-060 fork-copy v5** · **US-061 admin seams (isAdmin, routineCapOverride)** · hardening: undo soundness (§5.4), figures-route authz, non-destructive projection, post-connect role enforcement, snapshot catch-up + resend | **next up** — PLAN §9 "v5 migration milestone" |
...
| **FE-7 Custom kinds / lanes / search / sample** | extends `authoring.spec` + new specs — custom kind → editor; lanes; search; start-from-template | **US-043 custom kinds ✓ · US-044 lanes ✓ · US-045 template ✓ · US-046 search ✓ (PR #76)** · #18/#19/#81 | **built** — AddKindSheet + Lanes + search + start-from-template landed; `search.spec`/`template.spec`/`library.spec` + component/store tests green |
...
> **Status reconciled to code on 2026-06-28.** FE-3 (variants/COW — PR #74), FE-6 (annotations — PR #70), and FE-7 (custom kinds/lanes/search/template — PR #76) had all merged into `development` but were still listed "not started"; the table above now matches the code. Verified by running the full non-E2E suite on `development` HEAD — **domain 154 passed / 3 skipped, web 114 passed / 0 skipped, worker 101 passed / 7 skipped** (typecheck + lint clean). The `@smoke` E2E gate (per-PR via `ci.yml`, and now also on push via `deploy.yml`) is live — US-191 has landed. **Genuinely still open:** US-049 (Ops/Sentry/Analytics/EXPLAIN — M8, skipped), US-053 server `/api/profile` plan+count endpoint (skipped; UI shipped), US-054 full ISTD seed (Content, skipped), FE-8 PWA/offline/cross-browser (M9 — `pwa-a11y.spec` skipped), and the one annotations E2E slice noted in FE-6.
...
| US-008 🦴 | Copy-on-write (frozen choreo-owned copy) | M1 | system/developer | US-005 | `copyOnWrite` spawns a **frozen, choreo-owned copy** carrying its own attributes snapshot; `baseFigureRef` = provenance only (no overlay); placement re-pointed. *(Reconciled 2026-06; **⟳v5: superseded by US-057** — variants are live overlays.)* |
...
| US-033 | Personal-library figures + custom figures in library | M4 | user-facing | US-032 | See your personal-library figures (saved copies, lineage badge) + custom figures; two-scope badge (`library`/`custom`); "used in N routines". *(**⟳v5: superseded by US-059** — the library is a bookmark, not a copy.)* |
| US-034 | Editing your own figure flows into all referencing routines | M4 | user-facing | US-032,US-008 | Edit an owned figure → change appears in every routine referencing it (figure auto-update). *(**⟳v5: generalized** — any account figure is live everywhere, incl. for co-editors; the shipped behavior stands.)* |
| US-035 | Auto-copy on editing a figure from outside the choreo | M4 | user-facing | US-033,US-008 | Editing a global/personal-library figure silently creates a frozen choreo-owned copy, re-points placement, **"copied into this choreo"** toast; original untouched. *(**⟳v5: superseded by US-057/US-058** — only a global edit diverges, into a live variant.)* |
...
| US-057 ⟳v5 | Variant = live overlay (per-beat ownership + copy-down) | v5 | system/developer | US-005 | `resolveFigure(base, variant)`: owned beats read wholly from the variant, unowned beats wholly from the live base; editing an unowned beat copies the base's beat down first; spawn re-points the placement; the base is never mutated (PLAN §2.5.1 #14–18). |
| US-058 ⟳v5 | Global figures are real admin-owned docs | v5 | system/developer | US-014,US-021 | Additive seeder creates global figure DOs from the catalog; all signed-in users read, only admins write at the DO boundary; a new catalog placement is a **live reference**; the snapshot returns variant **bases**. |
| US-059 ⟳v5 | Library = per-user bookmark over shared docs | v5 | user-facing | US-058 | "Add to my library" records the figureRef (account doc `libraryFigureRefs` + LibraryEntry); a partner bookmarks the **same** doc from a shared choreo; un-bookmark removes the entry, never the doc. |
| US-060 ⟳v5 | Fork copies account figures, keeps catalog refs live | v5 | user-facing | US-057,US-037 | Fork = routine seed + a copy of each referenced account figure (variants stay variants — catalog flow-in continues); origin edits never reach the fork; admin catalog edits do. |
| US-061 ⟳v5 | Admin seams: `isAdmin` + `routineCapOverride` | v5 | system/developer | US-022 | `isAdmin` gates global-doc writes; the quota check honours `routineCapOverride`; both granted via ops until the admin UI (v1.1, D31). |
