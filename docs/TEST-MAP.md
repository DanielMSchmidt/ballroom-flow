...
| Honest error reporting *(2026-07-19, #275/#276)* | Authed-401 on an idempotent GET force-refreshes the token (`getToken({ skipCache: true })`) and retries once before reporting — heals the 20 s snapshot-poll expiry race *and* the false "Clerk instance mismatch" alarm; a still-401 refresh reports once. Network-throw reporting suppresses deliberate aborts + page teardown (`AbortError` / `visibilityState === "hidden"`), still reporting a genuine transport failure on a live page. Store-seam wiring proven on the default snapshot fetch. | unit (jsdom) | `apps/web/src/lib/rpc.test.ts` ("authed-401 fresh-token retry", "teardown vs genuine failure" describes), `apps/web/src/store/routine-snapshot.test.ts` ("authed-401 token refresh") |
...
| Staging demo seed *(US-056, 2026-07-20 — admin ops; docs/system/architecture.md § The staging demo seed)* | The pure `buildDemoSeed` (four cross-dance routines from real charted figures; annotations across every schema anchor type — point/figure/figureType; threaded replies; backdated createdAt recent→old; synthetic namespaced co-members that are routine members + note/family-note authors; a role-aware account-wide custom kind on steps; deterministic + user-namespaced ids, no fabrication) + the hard-gated `POST`/`DELETE /api/admin/seed-demo` (isAdmin gate before any write; own-account-only; revive-on-conflict idempotency; exact-id soft-delete; backdated timestamps preserved via `seedDoc` content) | domain + worker | `packages/domain/src/demo-seed.test.ts` (×8: multi-dance real figures, deterministic/namespaced, every anchor type, threaded replies, backdated spread, co-members + memberships + family notes, custom kind on steps, ownership), `apps/worker/src/routes/seed-demo.test.ts` (×10: 401/403 authz + no-write, materialization asserting routines/annotations/anchor-types/backdated createdAt/co-member reply/charted figure attrs/memberships/family notes/custom kind, idempotent re-run, soft-delete survives as tombstones, clean re-seed, EXPLAIN gate on the tombstone queries) |

| Comment activity fade-out *(2026-07-19 — reading view)* | Active comments (last 28d, plus anything within 7d of the newest activity in the list) render by default; the rest collapse behind ONE honest counted divider that expands in place; margin cells derive snippet/avatars from active comments only; reply reactivation; never-empty; family notes exempt; injected `now` (first wall-clock-dependent rendering) | domain + component + E2E | `packages/domain/src/annotation-activity.test.ts` (window edges both inclusive, reply reactivation, tombstoned replies ignored, properties: never-empty, newest active, order-preserving partition), `apps/web/src/components/comment-fadeout.test.tsx` (thread divider expand/collapse + honest header, all-recent/all-within-session no-divider, en/de strings, axe both states; active-only margin + family-note exemption), `apps/web/e2e/comment-activity-fadeout.spec.ts` (@smoke: comeback burst collapses/expands, quiet >28d cluster no divider — backdated via the seed seam) |

| Attribute-predicate anchors *(2026-07-19 — the fourth, dynamic anchor)* | `attributePredicate { kind, value, role?, scope }` surfaces one note on every step whose notation matches, re-evaluated on read; `none` absence sentinel; matched by meaning via `normalizeValue`; a new co-membership-gated D1 index mirroring the family-note index; the v4 § 3.6 attribute picker path grafted onto the choreo-first link picker; margin surfacing with referential stability | domain + worker + web + E2E | `packages/domain/src/predicate-anchor.test.ts` (the anchor variant + `zAnchor` round-trip + `addPredicateNote` + leniency regression), `packages/domain/src/predicate.test.ts` (`matchPredicate`: value/alias/`none`/role/unknown/tombstone/dynamic/variant + fast-check property), `apps/worker/src/predicate-projection.test.ts` (alarm projection: create/idempotent/tombstone/`routine`-scope), `apps/worker/src/predicate-visibility.test.ts` (**HARD GATE** — co-member 200, non-member 403-before-read, `routine`-scope never served, `expectIndexedQuery` no-SCAN), `apps/web/src/store/predicate-notes.test.ts` (loader + `mergePredicateNotes`), `apps/web/src/store/account.test.ts` + `use-account.test.tsx` (own self-read + `createPredicateNote`, offline, referential stability), `apps/web/src/components/predicate-picker.test.tsx` (the attribute path + `none` row + custom kind + Tango omits rise + axe), `apps/web/src/components/reading-view.test.tsx` ("predicate notes surface on matching steps" describe: value/`none`/routine-scope/referential-stability/axe), `apps/web/e2e/attribute-predicate-anchors.spec.ts` (@smoke: surfaces across two choreos + dynamic match; co-member sees, non-member blocked UI + 403) |

| AI voice notes *(2026-07-19 — speak a note, land it on the right anchor; 2026-07-20 — push-to-talk dual capture; 2026-07-20 — context-first scope-first capture; 2026-07-23 — the scope unit is the choreo, not the dance)* | Speak a practice note → a Workers AI text model resolves it against the figures ACTUALLY in your choreos into a **proposed** anchor (the three static shapes) → confirm/correct in the voice sheet → commit through the EXISTING annotation seams. Note creation is **scope-first**: pick a **choreo** (skippable → "all my dancing", remembered per-device, stale refs fall back) *before* capturing; the chosen choreo opens the link picker on the target step AND grounds the interpret route (`routineRef` on the request narrows `assembleVoiceContext` to that choreo, per-figure authz unchanged; the server-side `dance` narrowing remains supported/tested with no web caller since 2026-07-23). A dancer with **no choreos** sees the "create one in the Choreo tab" nudge; voice is not blocked (a plain dictated note stays valid — an empty scope is unselectable by construction). **Capture is push-to-talk** (hold to record, release to send; keyboard toggle for a11y) with a **dual on-device+recorder capture** (`dualCapture`): on-device transcript wins, else the recorded clip → Whisper — the fix for mobile Chrome, which streams no on-device results. Read-only `POST /api/voice-notes/{interpret,transcribe}`; a pure choreo-context serializer + dance aliases (domain); a mockable `VoiceAi` seam + deterministic fixture (zero-secret matrix); mandatory Zod re-validation + grounding of the model output (never a wrong anchor past confirm); Workers AI + AI Gateway on deployed envs only; predicate utterances fall back to a plain note | domain + contract + worker + web + E2E | `packages/domain/src/voice-context.test.ts` (`serializeChoreoContext` placement-order/tombstones/variant-resolution/counts + `resolveDanceAlias`, purity), contract cases in `packages/contract/src/index.test.ts` (interpret request/extraction/proposal schemas — three anchor shapes accepted, malformed model output rejected, **optional `dance` accepted + unknown dance rejected**), `apps/worker/src/voice-ai.test.ts` (`groundProposal` re-validation + grounding, deterministic fixture, `voiceAiFor`/`shouldUseWorkersAi` selection, `workersVoiceAi` model + gateway), `apps/worker/src/voice-context-scope.test.ts` (**dance-scoped grounding** via `SELF.fetch` — a Foxtrot Feather resolves under `dance:foxtrot`, is filtered out under `dance:waltz`, broad with no dance; a viewer-only routine of the scoped dance is excluded), `apps/web/src/lib/speech.test.ts` (`dualCapture`: on-device final wins, on-device-empty → audio fallback, `onerror` ignored, double-stop resolves once, fast-release edge, mic-denied error only without a recognizer), `apps/web/src/store/voice-notes.test.ts` (seam re-validates the wire), `apps/web/src/components/voice-note-sheet.test.tsx` (`proposalToLink` + push-to-talk hold→release→interpret + Whisper-fallback path + keyboard toggle + idle/recording/confirm states + Confirm payload per anchor shape + axe), `apps/web/src/components/journal.test.tsx` ("AI voice path" describe: Confirm routes through the existing family/routine seams, no save before confirm; **"context-first scope step" describe**: choreo list + skippable "all my dancing" + remembered choreo (stale-ref fallback), picker opens on target when a choreo is chosen, voice call carries `routineRef`, no-choreo nudge with voice still dictating), `apps/web/e2e/voice-notes.spec.ts` (@smoke ship gate, push-to-talk: feather family note surfaces on the sibling routine; bounce-fallaway instance-only; unresolved fallback), `apps/web/e2e/context-first-note.spec.ts` (@smoke ship gate: pick a choreo → voice proposal grounds within that choreo; the chosen choreo opens the link picker on the target step, manual path) |

| Annotation media embeds *(2026-07-19 — inline photos/videos/YouTube in notes)* | `Annotation.media[]` (image/video/youtube discriminated union, client ULIDs, soft-delete), placed inline by `![media:<id>]` tokens (`splitMediaParts`: live→embed, tombstoned/unknown→removed stub, unreferenced-live→appended); the FIRST binary storage (R2 `MEDIA` bucket per env) + a new authz surface (**HARD GATE**) — the object key `media/<docRef>/<annotationId>/<mediaId>` IS the scope. Worker-hosted upload-URL mint (commenter+ AND caps — image 10 MB / video 3 min & 300 MB / 4 items / 1 GB — in an indexed `media_object` counter), single-PUT + R2 multipart for large videos, stream-through serving with Range (viewer+; tombstoned still serves; `__session` cookie for element-src), worker-proxied YouTube facade thumb; media compose on the note composer (live-gated), inline render, compact chip on margin + Journal cards | domain + contract + worker + web + E2E | `packages/domain/src/media.test.ts` (`splitMediaParts` live/tombstoned/unknown/unreferenced/none; `attachMedia`/`softDeleteMedia` convergence: attach vs text-edit / vs annotation-tombstone / vs media-tombstone, soft-delete only), contract cases in `packages/contract/src/index.test.ts` (`zMintMediaUpload` + response round-trip, `MEDIA_CAPS`), `apps/worker/src/routes/media.test.ts` (**HARD GATE** — mint authz non-member/viewer 403 · caps 413/409/402 · single-PUT into R2 + over-grant 413 · PUT by non-owner/non-member 403 · multipart create/uploadpart/complete/abort under the same grant · serve viewer/owner 200 / non-member 403 / Range 206 & 416 / tombstoned still serves · youtube-thumb 401/403/400 + cache-control · `expectIndexedQuery` no-SCAN), `apps/worker/src/media-range.test.ts` (`parseRange`/`resolveRange` closed/open/suffix/garbage/unsatisfiable), `apps/worker/src/permissions.test.ts` ("refuses a commenter attaching MEDIA to another author's annotation"), `apps/worker/src/doc-do.test.ts` (journal projection media counts), `apps/worker/src/ops.test.ts` (the `MEDIA` binding in all four envs), `apps/web/src/lib/media-files.test.ts` (`youtubeVideoId` + `boundedDimensions`), `apps/web/src/store/media.test.ts` (`mintMediaUpload` POSTs + Zod-parses; `uploadMedia` single-PUT / multipart), `apps/web/src/components/annotation-media.test.tsx` (compose gating + pending chip; inline render order; facade no-iframe-until-tap; removed stub; video poster→player; margin/Journal chip has no img/video/iframe; axe), `apps/web/e2e/annotation-media.spec.ts` (@smoke ship gate: attach photo + YouTube inline → thread render + facade tap-to-load, no third-party until tap; a second member loads the photo, a non-member's direct fetch is 403) |

**Every live story (US-001…US-056, minus the retired US-047/US-048) is covered.** No story is left untested.
