...
| AI voice notes *(2026-07-19 — speak a note, land it on the right anchor)* | Speak a practice note → a Workers AI text model resolves it against the figures ACTUALLY in your choreos into a **proposed** anchor (the three static shapes) → confirm/correct in the voice sheet → commit through the EXISTING annotation seams. Read-only `POST /api/voice-notes/{interpret,transcribe}`; a pure choreo-context serializer + dance aliases (domain); a mockable `VoiceAi` seam + deterministic fixture (zero-secret matrix); mandatory Zod re-validation + grounding of the model output (never a wrong anchor past confirm); Workers AI + AI Gateway on deployed envs only; predicate utterances fall back to a plain note | domain + contract + worker + web + E2E | `packages/domain/src/voice-context.test.ts` (`serializeChoreoContext` placement-order/tombstones/variant-resolution/counts + `resolveDanceAlias`, purity), contract cases in `packages/contract/src/index.test.ts` (interpret request/extraction/proposal schemas — three anchor shapes accepted, malformed model output rejected), `apps/worker/src/voice-ai.test.ts` (`groundProposal` re-validation + grounding, deterministic fixture, `voiceAiFor`/`shouldUseWorkersAi` selection, `workersVoiceAi` model + gateway), `apps/worker/src/routes/voice-notes.test.ts` (**HARD GATE** — 401/400, scenario A/B/C, viewer-scope excluded, **read-only proof** via before/after row counts, `/transcribe` echo + 413), `apps/web/src/store/voice-notes.test.ts` (seam re-validates the wire), `apps/web/src/components/voice-note-sheet.test.tsx` (`proposalToLink` + rec/confirm/unresolved states + Confirm payload per anchor shape + axe), `apps/web/src/components/journal.test.tsx` ("AI voice path" describe: Confirm routes through the existing family/routine seams, no save before confirm), `apps/web/e2e/voice-notes.spec.ts` (@smoke ship gate: feather family note surfaces on the sibling routine; bounce-fallaway instance-only; unresolved fallback) |
