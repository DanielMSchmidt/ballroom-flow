...
| [0004](docs/proposals/0004-choreo-first-journal-links/README.md) | Choreo-first journal links with grid placement and scope-last anchoring | web, domain, contract, worker | implementable | Rebuild the journal link picker as one linear flow — choreo → type-ahead figure (choreo-scoped) → placement on the figure-detail attribute grid (entire figure or one count, with role lens) → scope last (all dances / this dance / this choreo). Adds optional `count`/`role` to the `figureType` anchor so timed notes can scope to this dance (never across dances). Read before touching the journal link picker or the `Anchor` union. |
