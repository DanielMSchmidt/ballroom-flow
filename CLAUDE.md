...
| [0002](docs/proposals/0002-account-doc-live-do/README.md) | Wire the account doc to a live Durable Object | worker, domain, web | implemented | Gave the per-user account doc (family notes + library bookmarks) a real DO like every other document: canonical CRDT state at `account:<userId>`, owner-only boundary, lazy `ensureAccountDoc` import from D1 rows, alarm projection back into `library_entry`/`figure_type_note_index` (write direction inverted), offline/undo via the shipped §11.2 machinery. Ship gate `apps/web/e2e/account-doc.spec.ts` green (@smoke). Read before touching family notes, bookmarks, `library_entry`, or `FigureTypeNoteIndex`. |
