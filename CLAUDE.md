...
| [0002](docs/proposals/0002-account-doc-live-do/README.md) | Wire the account doc to a live Durable Object | worker, domain, web | implementable | Give the per-user account doc (family notes + library bookmarks) a real DO like every other document: canonical CRDT state at `account:<userId>`, owner-only boundary, lazy `ensureAccountDoc` import from today's D1 rows, alarm projection back into `library_entry`/`figure_type_note_index` (inverting the write direction), offline/undo via the shipped §11.2 machinery. Design + test plan complete; ship gate `apps/web/e2e/account-doc.spec.ts`. Read before touching family notes, bookmarks, `library_entry`, or `FigureTypeNoteIndex`. |
...
| [0004](docs/proposals/0004-choreo-first-journal-links/README.md) | Choreo-first journal links with grid placement and scope-last anchoring | web, domain, contract, worker | implemented | Rebuild the journal link picker as one linear flow — choreo → type-ahead figure (choreo-scoped) → placement on the figure-detail attribute grid (entire figure or one count, with role lens) → scope last (all dances / this dance / this choreo). Adds optional `count`/`role` to the `figureType` anchor so timed notes can scope to this dance (never across dances). Read before touching the journal link picker or the `Anchor` union. |
| [0005](docs/proposals/0005-annotation-media-embeds/README.md) | Embed photos, videos, and YouTube links in annotations | domain, contract, worker, web, design, ops | provisional | Multimedia in comments: R2-uploaded photos/videos + YouTube embeds, placed **inline in the note text** via id tokens; full embeds render only in the opened thread — the reading-view margin and Journal cards show a compact media chip that opens it. Picks up PLAN §13 / Q-M1/2/3 (types, caps, entities). Read before touching annotations, the notes margin, or anything media/R2. |
| [0006](docs/proposals/0006-ws-heartbeat/README.md) | WS heartbeat — detect half-open ("zombie") sync sockets | web, worker, contract | implemented | Client-sent idle ping (`ballroom:sync:ping`) answered by a DO-level auto-response pong without waking a hibernating DO; a missed pong deadline drops the socket into the existing warm-reconnect machinery, bounding how long a silently dead connection can impersonate "live" (~30 s instead of the OS TCP timeout). Ship gate `apps/web/e2e/zombie-socket.spec.ts` (green 2026-07-13). Read before touching the sync wire or liveness detection. |
| [0007](docs/proposals/0007-comment-activity-fadeout/README.md) | Comment activity fade-out in the timeline reading view | web, domain, design | provisional | Show only "active" comments in the reading view's notes margin + threads — active = thread activity within 28 days OR within 7 days of the list's newest comment (session-gap window; never renders empty) — collapsing the rest behind a counted "N more comments" expander. Presentation-only: no data-shape change, nothing deleted, no read/unread state. Read before touching the notes margin, thread panel, or annotation display. |
| [0008](docs/proposals/0008-role-scoped-step-editing/README.md) | Role-scoped step editing with a Both write mode | domain, web | implemented | Edit-mode STEPS FOR lens gains **Both** and becomes the write scope: single-role writes are role-tagged (fixing the `role: null` quick-add leak), Both writes direction/sway mirrored for the follower (forward↔back, to_L↔to_R), footwork leader-only, all other kinds shared; hand-diverged values are locked under Both. Read lens stays Leader/Follower. Ship gate `apps/web/e2e/role-steps.spec.ts`. Read before touching the role lens, AttributeEditor write path, or `filterByRoleView`. |
| [0009](docs/proposals/0009-ai-voice-notes/README.md) | AI voice notes — speak a note, land it on the right anchor | web, worker, contract, domain, ops | implementable | Speak a practice note ("in Slowfox, in Feather Steps, settle the sway") → on-device STT (`@cf/openai/whisper-large-v3-turbo` fallback) → a **Cloudflare Workers AI** text model (default `@cf/meta/llama-3.3-70b-instruct-fp8-fast`, JSON mode) given the **in-scope choreo serialized structurally**, proposes a WEP-0004 anchor (`figureType`/`figure`/`point`) + note text → user confirms in the existing picker → committed through the existing `createAnnotation`/`createFamilyNote` seam. AI **proposes only**; no new data shape, no new trust surface, Cloudflare-only (escalation stays in-Cloudflare; sufficiency validated in the field); unresolved → transcribe-only. Ship gate `apps/web/e2e/voice-notes.spec.ts`. Read before touching voice capture, the interpret route, or annotation pre-fill. |
...
- **Keep types honest — a cast is a claim the compiler can't check, so it's a latent bug.** A `as X`, `as unknown as X`, `any`, non-null `!`, or `@ts-expect-error` doesn't fix a type problem, it *silences* it: if reality later diverges from the claim, nothing catches it. Treat every cast as debt with a burden of proof. **Machine-enforced since 2026-07-13** (`pnpm lint`): a GritQL plugin (`lint-plugins/no-type-assertion.grit`) errors on every `as`/`<T>` assertion (`as const` allowed), Biome errors on `any`/`!`/`@ts-ignore`, and `scripts/check-type-suppressions.mjs` bans `@ts-expect-error`/`@ts-nocheck` and confines `biome-ignore lint/plugin:` bypasses to allowlisted helpers (see `docs/TOOLING.md` § Type-honesty enforcement).
