...
// tombstone + store.deleteAnnotation) already existed; no component exposed it.
// AnnotationPanel now offers a per-annotation delete on the viewer's OWN
// annotations only (docs/concepts/annotations.md § Ownership: "anyone may only
// edit or delete their own"), mirroring the reply-delete / media-remove gate.
...
describe("annotation delete — author gate (standard mode)", () => {
  ...
  it("offers NO delete control on a co-member's annotation in standard mode", async () => {
    const { AnnotationPanel } = await importComponent<AnnotationPanelModule>(
      "../components/AnnotationPanel",
    );
    renderUi(
